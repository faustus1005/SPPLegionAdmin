...
--
-- SPP Legion Admin
-- SPP Legion Admin is a derivative of MarsAdmin, which is a derivative of TrinityAdmin.
--
...
--
-- Repository: https://github.com/faustus1005/SPPLegionAdmin
-------------------------------------------------------------------------------------------------------------
...
MAJOR_VERSION = "SPP Legion Admin V1"
MINOR_VERSION = "$Revision: 1 $"
ROOT_PATH     = "Interface\\AddOns\\SPPLegionAdmin\\"
local cont = ""
...
function MangAdmin:UpdateHeaderInfo()
  local guid = UnitGUID("player") or ""
  local realm = GetRealmName() or ""
  ma_toptext:SetText(Locale["char"].." "..Locale["guid"]..guid)
  ma_top2text:SetText(Locale["realm"]..realm)
end
...
function MangAdmin:ShowMainFrame()
  FrameLib:HandleGroup("bg", function(frame) frame:Show() end)
  local tab = self.db.char.activeTab or "main"
  self.db.char.activeTab = tab
  MangAdmin:ToggleTabButton(tab)
  MangAdmin:ToggleContentGroup(tab)
end
...
function MangAdmin:AddMessage(frame, text, r, g, b, id)
  ...
  local showServerInfoOutput = MangAdmin.db.account.style.showchat and (MangAdmin.db.account.style.showserverinfochat ~= false)
...
--        output = MangAdmin.db.account.style.showchat
    ...
    local matchedOnlinePlayers = false
    local onlineText = string.gsub(text, "|c%x%x%x%x%x%x%x%x", "")
    onlineText = string.gsub(onlineText, "|r", "")
    if string.find(onlineText, "Core rev.", 1, true)
      or string.find(onlineText, "World delay:", 1, true)
      or string.find(onlineText, "Map delay:", 1, true)
      or string.find(onlineText, "Session delay:", 1, true) then
      catchedSth = true
      output = showServerInfoOutput
    end
    ...
    for users, maxusers in string.gmatch(onlineText, Strings["ma_GmatchOnlinePlayers"]) do
      users = string.gsub(users, ",", "")
      maxusers = string.gsub(maxusers, ",", "")
...
--        output = MangAdmin.db.account.style.showchat
    ...
    if not matchedOnlinePlayers then
      local fallbackPatterns = {
        "Players online: ([%d,]+) %(max:?%s*([%d,]+)[^%)]*%)",
        "Players Online: ([%d,]+) %(max:?%s*([%d,]+)[^%)]*%)",
        "Players online: ([%d,]+) %(Max:?%s*([%d,]+)[^%)]*%)",
        "Players Online: ([%d,]+) %(Max:?%s*([%d,]+)[^%)]*%)",
      ...
      for _, pattern in ipairs(fallbackPatterns) do
        ...
        if matchedOnlinePlayers then
          break
        end
      end
      if not matchedOnlinePlayers then
        local loweredOnlineText = string.lower(onlineText)
        if string.find(loweredOnlineText, "online") and string.find(loweredOnlineText, "max") then
          local numbers = {}
          for number in string.gmatch(onlineText, "(%d+)") do
            table.insert(numbers, number)
          end
          if numbers[1] and numbers[2] then
            ma_infoonlinetext:SetText(Locale["info_online"]..numbers[1])
            ma_infomaxonlinetext:SetText(Locale["info_maxonline"]..numbers[2])
            matchedOnlinePlayers = true
            catchedSth = true
            output = showServerInfoOutput
          end
...
function MangAdmin:InitDropDowns()
  ...
  UIDropDownMenu_SetWidth(ma_languagedropdown, 60)
    ...
    if count > 0 then
      ...
      for line = 1,7 do
        ...
        if lineplusoffset <= count then
          ...
          --item icons
		  lineitem = getglobal("ma_PopupScrollBarEntryIcon"..line)
          getglobal("ma_PopupScrollBarEntryIcon"..line.."IconTexture"):SetTexture(GetItemIcon(item["itId"]))
          getglobal("ma_PopupScrollBarEntryIcon"..line):SetScript("OnEnter", function() GameTooltip:SetOwner(lineitem, "ANCHOR_RIGHT"); GameTooltip:SetHyperlink("item:"..item["itId"]..":0:0:0:0:0:0:0"); GameTooltip:Show() end)
          getglobal("ma_PopupScrollBarEntryIcon"..line):SetScript("OnLeave", function() GameTooltip:SetOwner(lineitem, "ANCHOR_RIGHT"); GameTooltip:Hide() end)
          ...
          --item description
		  lineitem = getglobal("ma_PopupScrollBarEntry"..line)
          ...
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnEnter", function() GameTooltip:SetOwner(lineitem, "ANCHOR_RIGHT"); GameTooltip:SetHyperlink("item:"..item["itId"]..":0:0:0:0:0:0:0"); GameTooltip:Show() end)
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnLeave", function() GameTooltip:SetOwner(lineitem, "ANCHOR_RIGHT"); GameTooltip:Hide() end)
...
function MangAdmin:InitCheckButtons()
  ...
  ma_showserverinfochatbutton:SetChecked(self.db.account.style.showserverinfochat ~= false)
